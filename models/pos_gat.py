...
class PoS_GAT(nn.Module):
    def __init__(self, embedding_matrix,dependency_matrix, opt):
        super(PoS_GAT, self).__init__()
        self.opt = opt
        self.number = 2
        self.edge_voc_number = dependency_matrix.shape[0]
        self.edge_voc_dim = dependency_matrix.shape[1]
        ...
        self.edge_embed=nn.Embedding.from_pretrained(
                torch.tensor(dependency_matrix, dtype=torch.float),freeze=True,padding_idx=0)
        # self.edge_embed=nn.Embedding(num_embeddings=self.edge_voc_number,embedding_dim=self.edge_voc_dim,padding_idx=0)
        # print(self.edge_embed)
        # exit()
        ...
        self.gat4 = GraphLayer(opt,opt.hidden_dim, opt.hidden_dim,dropout =opt.graph_dropout,alpha=opt.GAT_alpha,n_heads=int(opt.hidden_dim/100))
        ...
        self.fc3 = nn.Linear(opt.hidden_dim * 2, opt.polarities_dim)
    ...
    def forward(self, inputs):
        [text_indices, aspect_indices, left_indices,adj,unadj,dependency_type_matrix,speech_list],flage = inputs
        ...
        x1 = torch.relu(self.gat1(x_position_out , adj,edge,aspect_double_idx))
        #
        # #no position
        ...
        # x1 = self.syntactic_distance_position_weight(x1,aspect_double_idx, text_len, aspect_len, seq_len,adj)
        x2 = torch.relu(self.gat2(x1, adj,edge,aspect_double_idx ))
        # x2 = self.syntactic_distance_position_weight(x2,aspect_double_idx, text_len, aspect_len, seq_len,adj)
        x3 = torch.relu(self.gat3(x2, adj,edge,aspect_double_idx ))
        # x4 = torch.relu(self.gat4(x3, adj, edge, aspect_double_idx))
        x_graph=x3
        # x_graph =(x1)
        ...
        fnout = torch.cat((a1,graph_max_pool), 1)
