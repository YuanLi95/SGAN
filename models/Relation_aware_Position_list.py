...
class Muti_GAT(nn.Module):
    ...
    def forward(self,x, adj_type,edge_matrix,aspect_double_idx):
        ...
        for i in range(self.n_heads):
            h_edge_i, attention_edge_i, = self.attentions[i](x,adj_type,edge_matrix,aspect_double_idx)
        ...
        attention_edge = torch.mul(attention_edge,aspect_attention).mean(dim=0).squeeze()
...
class Relation_aware_Pos(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range (self.Syn_Layer):
            ...
            x,e_final = self.gat_layer_list[i](x,adj_type,edge_embedding,aspect_double_idx)
            # print(x)


            x1 = torch.mul(x,aspect_postion)
            if i !=(self.Syn_Layer-1):
        ...
        graph_mask = self.mask(x, aspect_double_idx)
