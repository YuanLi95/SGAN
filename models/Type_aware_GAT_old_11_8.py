...
class Muti_GAT(nn.Module):
    ...
    def forward(self,x, adj_type,edge_matrix,aspect_double_idx):
        ...
        # out_gat = F.dropout(H_prime, self.dropout, training=self.training)  # dropout，防止过拟合
...
class Type_aware_GAT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for index,(adj_type,edge_matrix) in enumerate([(unadj,dependency_type_matrix_undir)]):
            ...
            # x_graph = x3
        x_graph = torch.stack([x4]).squeeze()
        ...
        for i in range(self.hop):
            ...
            concat_final = torch.cat((text_out_s__mask,aspect_embdding),dim=2)
        ...
        fnout = torch.cat((before_aspect,a3), -1)
